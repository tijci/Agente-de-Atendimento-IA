...
const SYSTEM_PROMPT = [
    ...
    "",
    "REGRA DE LINK DE PORTAL (CRÍTICA):",
    "- Se a última mensagem do cliente contiver a marcação \"[LINK DE PORTAL IMOBILIÁRIO]\" ou uma URL de portal imobiliário (como imovelweb, zapimoveis, vivareal, olx), classifique a intenção OBRIGATORIAMENTE como \"ALUGAR\" (se o link ou texto contiver \"aluguel\", \"alugar\", \"locacao\" ou \"locar\") ou \"COMPRAR\" (caso contrário).",
    "- Não envie nenhuma mensagem de resposta ('reply' deve ser null) pois o SDR tratará o link buscando no banco de dados."
].join("\n");

export const receptionNode = async (state: typeof AgentState.State) => {
    if (state.currentAgent === 'SDR' || state.currentAgent === 'CAPTADOR') {
        return { intent: state.intent, currentAgent: state.currentAgent };
    }
